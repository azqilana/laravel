...
@section('container')
    ...
    @if ($posts->count())
        <div class="card mb-3" >
            {{-- <img src="https://api.unsplash.com/photos?query={{ {{ $posts[0]->kategory->name }} }}" class="card-img-top" alt="..."> --}}
            <div class="card-body text-center">
                <h5 class="card-title">{{ $posts[0]->judul }}</h5>
                <small>
                <p>Dari <a href="/penulis/{{ $posts[0]->user->name }}">{{ $posts[0]->user->name }}</a> in <a href="/kategories/{{ $posts[0]->kategory->slug }}" style="font-size: large">{{ $posts[0]->kategory->name }}</a><a> {{ $posts[0]->created_at->diffForHumans() }}</a></p>
                </small>
    ...
    @foreach ($posts->skip(1) as $p)
            <div class="col-md-4">
                ...
                <div class="card-body">
                    ...
                    <p>Dari <a href="/penulis/{{ $p->user->name }}">{{ $p->user->name }}</a></p>
